...
/*---------------------------------------------------------------------------*/
/* chip interface                                                                 */ 
cyg_uint32 gx_chip_probe(void)  __THROW;

int printk( const char *fmt, ... );

void cyg_dcache_off(void)  __THROW;
void cyg_dcache_on(void)  __THROW;

void cyg_time_delay_us(volatile cyg_uint32 delay)   __THROW;   

struct gx_malloc_info {
	int heap_total;		
	int heap_blocks;			
	int heap_allocated;		
	int heap_free;				
...
#ifdef CYGPKG_HAL_CK_CK510
#define BOOT_ROOTFS_SAVE_BASE    0X80800000
#endif
...
#ifdef CYGPKG_HAL_CK_CK610
#define BOOT_ROOTFS_SAVE_BASE    0Xc0800000
#endif

#ifdef CYGPKG_HAL_ARM
#define BOOT_ROOTFS_SAVE_BASE    0Xc0004000
#endif 

int gx_lowpower_entry(cyg_uint32 WakeTime,cyg_uint32 GpioMask,cyg_uint32 GpioData,cyg_uint32 key,cyg_uint8 *cmdline);

extern struct gx_macinfo *gx_mac_bd_get(void);

#ifdef GoXtend_ECOS
externC void gxos_interrupt_mask(cyg_vector_t vector);

externC void gxos_interrupt_unmask(cyg_vector_t vector);

externC void gxos_interrupt_acknowledge(cyg_vector_t vector);
#endif
/*---------------------------------------------------------------------------*/
#ifdef __cplusplus
}
