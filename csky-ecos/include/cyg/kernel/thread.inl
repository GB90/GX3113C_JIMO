...
inline void Cyg_HardwareThread::check_stack(void)
{
    ...
    for ( i = 0;
        ...
        if ((sig ^ (i * 0x01010101)) != base[i]) {
            ...
            diag_printf("stack_base:0x%x  stack_size:0x%x  stack_ptr:0x%x  entry_point:0x%x\n", \
			    (cyg_uint32)stack_base, (cyg_uint32)stack_size, (cyg_uint32)stack_ptr, (cyg_uint32)entry_point);
	    CYG_ARCH_PRIVATE_ERROR_PROCESS(reason);
