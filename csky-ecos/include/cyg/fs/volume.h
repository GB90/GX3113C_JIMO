...
typedef struct ntfs_dir_fd_s {
	ntfs_volume *vol;
	ntfs_status_t status;
	s64 pos;
} ntfs_dir_fd_t;
...
typedef struct ntfs_fd_s {
	ntfs_volume *vol;
	ntfs_status_t status;
	cyg_bool update;	//Mark "update" flag.
} ntfs_fd_t;

typedef struct ntfs_fsearch_s {
	struct list_head list;
	u32 usecnt;		//Fd count.
	u64 mft_no;		//Inode number.
	ntfs_inode *ni_write;	//Store write inode.
	cyg_bool dirty;		//Inode has been written.
} ntfs_fsearch_t;
...
struct _ntfs_volume {
	...
	u32 ntfs_mounted;
	ssize_t total_flush;
	cyg_drv_mutex_t wr_lock;
	struct dir dir_head;
	ntfs_fsearch_t fs_head;
};
