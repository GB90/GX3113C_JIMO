...
#include <cyg/hal/ck610m_cache.h>
// Use this macro to allow the assembler to accept "cache" instructions,
// which are CK ISA 3. This is useful if someone is compiling

#define _hal_asm_ck_cpp_stringize( _x_ ) #_x_
#define _HAL_ASM_SET_CK_ISA( _isal_ ) asm volatile ( \
      ".set ck" _hal_asm_ck_cpp_stringize(_isal_) )


//=============================================================================
// Default Implementation. This uses the standard CK CP0 registers and
...
// Defines for various R4000+ CK cache operations
// this is a 5 bit field with bits 4-2 defining the operation
// and bits 1-0 defining which cache is being operated on
#define HAL_CK_CACHE_INDEX_INVALIDATE_I    0x00 /* 0 0 */
#define HAL_CK_CACHE_INDEX_WRITEBACK_INV_D 0x01 /* 0 1 */
#define HAL_CK_CACHE_INDEX_WRITEBACK_INV_S 0x03 /* 0 3 */
#define HAL_CK_CACHE_INDEX_LOAD_TAG_I      0x04 /* 1 0 */
#define HAL_CK_CACHE_INDEX_LOAD_TAG_D      0x05 /* 1 1 */
#define HAL_CK_CACHE_INDEX_LOAD_TAG_S      0x07 /* 1 3 */
#define HAL_CK_CACHE_INDEX_STORE_TAG_I     0x08 /* 2 0 */
#define HAL_CK_CACHE_INDEX_STORE_TAG_D     0x09 /* 2 1 */
#define HAL_CK_CACHE_INDEX_STORE_TAG_S     0x0b /* 2 3 */
#define HAL_CK_CACHE_HIT_INVALIDATE_I      0x10 /* 4 0 */
#define HAL_CK_CACHE_HIT_INVALIDATE_D      0x11 /* 4 1 */
#define HAL_CK_CACHE_HIT_INVALIDATE_S      0x13 /* 4 3 */
#define HAL_CK_CACHE_FILL_I                0x14 /* 5 0 */
#define HAL_CK_CACHE_HIT_WRITEBACK_INV_D   0x15 /* 5 1 */
#define HAL_CK_CACHE_HIT_WRITEBACK_INV_S   0x17 /* 5 3 */

// there seems to be different uses for operation
// code 6 depending on CPU
#define HAL_CK_CACHE_HIT_WRITEBACK_D       0x19 /* 6 1 */

#define HAL_CK_CACHE_INDEX_LOAD_DATA_I     0x18 /* 6 0 */
#define HAL_CK_CACHE_INDEX_LOAD_DATA_D     0x19 /* 6 1 */
#define HAL_CK_CACHE_INDEX_LOAD_DATA_S     0x1b /* 6 3 */

// there seems to be different uses for operation
// code 7 depending on CPU
#define HAL_CK_CACHE_FETCH_AND_LOCK_I      0x1c /* 7 0 */
#define HAL_CK_CACHE_FETCH_AND_LOCK_D      0x1d /* 7 1 */

#define HAL_CK_CACHE_INDEX_STORE_DATA_I    0x1c /* 7 0 */
#define HAL_CK_CACHE_INDEX_STORE_DATA_D    0x1d /* 7 1 */
#define HAL_CK_CACHE_INDEX_STORE_DATA_S    0x1f /* 7 3 */
...
#ifdef HAL_CK_CACHE_INSN_USES_LSB
...
#define HAL_DCACHE_SYNC()                                               \
    ...
    _HAL_ASM_SET_CK_ISA(3);                                             \
    for( ; _addr_ <= _baddr_+_size_; _addr_ += HAL_DCACHE_LINE_SIZE )   \
    { _HAL_ASM_DCACHE_ALL_WAYS(HAL_CK_CACHE_INDEX_WRITEBACK_INV_D, _addr_); } \
    _HAL_ASM_SET_CK_ISA(0);                                           \
    CYG_MACRO_END
#endif
...
#define HAL_DCACHE_LOCK(_base_, _asize_)                                    \
    ...
    if( _state_ ) {                                                         \
        ...
        { _HAL_ASM_DCACHE_ALL_WAYS(HAL_CK_CACHE_FETCH_AND_LOCK_D, _addr_); }\
        _HAL_ASM_SET_CK_ISA(0);                                             \
    }                                                                       \
    CYG_MACRO_END
#endif
...
#define HAL_DCACHE_FLUSH( _base_ , _asize_ )                                \
    ...
    if( _state_ ) {                                                         \
        _HAL_ASM_SET_CK_ISA(3);                                             \
        for( ; _addr_ < _eaddr_; _addr_ += HAL_DCACHE_LINE_SIZE )           \
        { _HAL_ASM_DCACHE_ALL_WAYS(HAL_CK_CACHE_HIT_WRITEBACK_INV_D, _addr_); } \
        _HAL_ASM_SET_CK_ISA(0);                                             \
    }                                                                       \
    CYG_MACRO_END
#endif
...
#define HAL_DCACHE_INVALIDATE( _base_ , _asize_ )                           \
    ...
    _HAL_ASM_SET_CK_ISA(3);                                                 \
    for( ; _addr_ < _eaddr_; _addr_ += HAL_DCACHE_LINE_SIZE )               \
    { _HAL_ASM_DCACHE_ALL_WAYS(HAL_CK_CACHE_HIT_INVALIDATE_D, _addr_); }    \
    _HAL_ASM_SET_CK_ISA(0);                                                 \
    CYG_MACRO_END
#endif
...
#define HAL_DCACHE_STORE( _base_ , _asize_ )                                \
    ...
    if( _state_ ) {                                                         \
        _HAL_ASM_SET_CK_ISA(3);                                             \
        for( ; _addr_ < _eaddr_; _addr_ += HAL_DCACHE_LINE_SIZE )           \
        { _HAL_ASM_DCACHE_ALL_WAYS(HAL_CK_CACHE_HIT_WRITEBACK_D, _addr_); } \
        _HAL_ASM_SET_CK_ISA(0);                                             \
    }                                                                       \
    CYG_MACRO_END
#endif
...
CYG_MACRO_START                                                          \
    ...
    (_state_) = (0 != (0x100 & reg)); /* Bit 12 is ICache enable */      \
CYG_MACRO_END
#endif
...
#define HAL_ICACHE_INVALIDATE_ALL()                                           \
    ...
    { _HAL_ASM_ICACHE_ALL_WAYS(HAL_CK_CACHE_INDEX_INVALIDATE_I, _addr_); }    \
    _HAL_ASM_SET_CK_ISA(0);                                                   \
    CYG_MACRO_END
#endif
...
#define HAL_ICACHE_LOCK(_base_, _asize_)                                    \
    ...
    if( _state_ ) {                                                         \
        ...
        { _HAL_ASM_ICACHE_ALL_WAYS(HAL_CK_CACHE_FETCH_AND_LOCK_I, _addr_); }\
        _HAL_ASM_SET_CK_ISA(0);                                             \
    }                                                                       \
    CYG_MACRO_END
#endif
...
#define HAL_ICACHE_INVALIDATE( _base_ , _asize_ )                           \
    ...
    _HAL_ASM_SET_CK_ISA(3);                                                 \
    for( ; _addr_ < _eaddr_; _addr_ += HAL_ICACHE_LINE_SIZE )               \
    { _HAL_ASM_ICACHE_ALL_WAYS(HAL_CK_CACHE_HIT_INVALIDATE_I, _addr_); }    \
    _HAL_ASM_SET_CK_ISA(0);                                                 \
    CYG_MACRO_END
#endif
...
#error Unsupported CK configuration

#endif
//--------------------------added by gaohb start------------------------------->
...
#define CYG_CACHES_ON(_d_, _i_)           \
    CYG_MACRO_START                             \
    if (_d_) HAL_DCACHE_ENABLE();               \
    if (_i_) HAL_ICACHE_ENABLE();               \
    CYG_MACRO_END

#define CYG_DCACHES_OFF(_d_)          \
    CYG_MACRO_START                             \
    HAL_DCACHE_IS_ENABLED(_d_);                 \
    HAL_DCACHE_SYNC();                          \
    HAL_DCACHE_INVALIDATE_ALL();                \
...
#define CYG_DCACHES_ON(_d_)           \
    CYG_MACRO_START                             \
    if (_d_) HAL_DCACHE_ENABLE();               \
    CYG_MACRO_END

#define CYG_DCACHES_CLEAN()          \
    CYG_MACRO_START                             \
    HAL_DCACHE_SYNC();                          \
    HAL_DCACHE_INVALIDATE_ALL();                \
    CYG_MACRO_END
    
#define CYG_ICACHES_OFF(_i_)          \
    CYG_MACRO_START                             \
    HAL_ICACHE_IS_ENABLED(_i_);                 \
    HAL_ICACHE_INVALIDATE_ALL();                \
    HAL_ICACHE_DISABLE();                       \
