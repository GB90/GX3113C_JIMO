...
struct cyg_nand_dev_fns_v2 {
...
#if 0
	int (*read_with_ecc)(cyg_nand_device *dev, cyg_nand_page_addr page, 
			void * dest, size_t size,size_t nbytes,
			CYG_BYTE *ecc_pos);
#endif
	...
	int (*read_oob)(cyg_nand_device *dev,  void * spare, size_t spare_size);
...
struct _cyg_nand_device_t {
	...
	size_t chip_bits; /* log2 of total chip size in BYTES. */
	...
	cyg_bool read;                 //Gx ecc hardware init is different from read and write.
	cyg_uint32 column_addr; //Large page devided into 4 small part,thus address should be changed.
	cyg_uint32 nand_base;    //Nand flash register base addess.
	cyg_bool use_ecc;
} CYG_HAL_TABLE_TYPE;
