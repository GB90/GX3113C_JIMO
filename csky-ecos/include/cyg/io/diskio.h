...
struct  cyg_disk_partition_t
{
    ...
    cyg_uint32 size2;   // size in sectors. the high 32bit of ntfs partition size(64bit) 
};

struct  cyg_disk_info_t
{
    struct cyg_disk_partition_t *partitions;    // partition table
    int                         partitions_num;// partition table size
    struct cyg_disk_identify_t  ident;         // identify data
    ...
    int							total_partition_counts;	//total partition num of the disk
	int 						current_partition_index;//current partition number
};

struct cyg_disk_part_info_t
{
    char devname[32];       // name of hardware device
    char pathname[32];      // name of mount point
    char fsname[8];        // name of implementing filesystem
    cyg_uint32 block_size;
    cyg_uint64 total_size;
    cyg_uint64 freed_size;
    cyg_uint64 used_size;
};
