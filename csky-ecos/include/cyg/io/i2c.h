...
typedef struct cyg_i2c_bus {
	...
	void					(*i2c_clk_cfg_fn)(const cyg_i2c_device*, cyg_uint32);
    // A spare field for use by the driver
    void*                   i2c_extra;
} CYG_HAL_TABLE_TYPE cyg_i2c_bus;

#define CYG_I2C_BUS(_name_, _init_fn_, _tx_fn_, _rx_fn_, _stop_fn_, _clk_cfg_fn, _extra_)    \
    cyg_i2c_bus _name_  CYG_HAL_TABLE_ENTRY( i2c_buses ) = {                    \
        .i2c_init_fn    = _init_fn_,                                            \
        .i2c_tx_fn      = _tx_fn_,                                              \
        .i2c_rx_fn      = _rx_fn_,                                              \
        .i2c_stop_fn    = _stop_fn_,                                            \
        .i2c_clk_cfg_fn = _clk_cfg_fn,										\
        .i2c_extra      = _extra_                                               \
    }
