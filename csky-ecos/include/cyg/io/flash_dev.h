...
struct cyg_flash_dev_funs {
	...
	int     (*flash_read) (struct cyg_flash_dev *dev, 
			...
			void* data, size_t len);
	int     (*flash_info) (struct cyg_flash_dev *dev, 
			char* info);
...
struct cyg_flash_otp_dev {
	cyg_uint32  start_addr;
	cyg_uint32  skip_addr;
	cyg_uint32  region_size;
	cyg_uint32  region_num;
	cyg_uint32  flags;
};
...
#else
# define CYG_FLASH_FUNS(_funs_, _init_, _query_ , _erase_, _prog_ , _read_, _info_,\
		_otp_enter_, _otp_lock_, _otp_status_, _otp_erase_, _otp_read_, _otp_write_,\
		_otp_get_region_, _otp_set_region_)\
struct cyg_flash_dev_funs _funs_ =      \
{										\
	.flash_init             = _init_,        \
