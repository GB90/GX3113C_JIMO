...
typedef enum
{
	CLUSTER_SIZE_4096 = 4096,
	CLUSTER_SIZE_8192 = 8192,
	CLUSTER_SIZE_16K = (16*1024),
	CLUSTER_SIZE_32K = (32*1024),
	CLUSTER_SIZE_64K = (64*1024),
}cluster_size_t;

struct mkfs_options {	
	cluster_size_t cluster_size;		/* -c, format with this cluster-size */
...
typedef int     cyg_fsop_setinfo  ( cyg_mtab_entry *mte, cyg_dir dir, const char *name,
                                    int key, void *buf, int len );
typedef int     cyg_fsop_mkfs  ( const char *devname, struct mkfs_options *opts);
...
#define FS_INFO_VOLNAME			8		/* get disk volue name */
#define FS_INFO_FSTYPE          9       /* get file system type */
...
typedef CYG_ADDRWORD cyg_fileop_mmap  (struct CYG_FILE_TAG *fp, CYG_ADDRWORD data, int len); //gxcoreapi modified
...
struct CYG_FILEOPS_TAG
{
	...
	cyg_fileop_mmap		*fo_mmap;	//gxcoreapi modified
};
...
__externC int mkfs( const char *devname,
                     const char *fsname, struct mkfs_options *opts);
...
__externC int fstab_register(struct cyg_fstab_entry *ftab);
