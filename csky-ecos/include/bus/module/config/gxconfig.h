...
/**执行成功*/
#define     GXCONFIG_OK                     (0)
/**执行失败*/
#define     GXCONFIG_FAILURE                (-1)
/**输入参数错误*/
#define     EINPUT                          (-2)
/**值最大长度*/
#define     MAX_CONFIG_VALUE_LEN            (64)
/**配置文件名*/
#define     GXBUS_CONFIG_FILE_NAME          "/home/gx/config.ini"
/**配置文件备份名*/
#define     GXBUS_CONFIG_FILE_BACKUP        "/home/gx/config.ini.bak"
/**文件完整性保护字符串*/
#define     GXBUS_FILE_PROTECT              "end"
/**Section与Parameter字符串间隔符*/
 ...
 */
char*       GxBus_ConfigGet                     (const char*    key, 
                                                 char*          buf, 
                                                 size_t         buf_size,
                                                 const char*    dvalue);
/**
 ...
 * @remarks     获取key标识的变量的值时，存在两种可能情况，即key变量在配置表中存
                ...
                并将在配置表中增加变量key，并以dvalue为变量key的值写入配置表
 */  
int32_t*    GxBus_ConfigGetInt                  (const char*    key, 
                                                 int32_t*       buf, 
                                                 int32_t        dvalue);
/**
 * @brief       从配置表中获取key标识的变量的值
 ...
 * @remarks     获取key标识的变量的值时，存在两种可能情况，即key变量在配置表中存
                ...
                并将在配置表中增加变量key，并以dvalue为变量key的值写入配置表
 */                                                                             
double*     GxBus_ConfigGetDouble               (const char*    key, 
                                                 double*        buf, 
                                                 double         dvalue);
/**
 ...
 */
int32_t     GxBus_ConfigSet                     (const char*    key, 
                                                 const char*    value);
/**
 * @brief       设置key标识的变量的值到配置表
 * @param [in]  key         变量标识字符串
 ...
 * @remarks     如果设置的key在配置表中不存在，则函数将在配置表中增加这个KEY
 */
int32_t     GxBus_ConfigSetInt                  (const char*    key, 
                                                 int32_t        value);
 ...
 * @remarks     如果设置的key在配置表中不存在，则函数将在配置表中增加这个KEY
 */
int32_t     GxBus_ConfigSetDouble               (const char*    key, 
                                                 double         value);
/**
 * @brief       打开配置表组
 * @param       void
 ...
 */
int32_t     GxBus_ConfigGroupSet                (handle_t       group, 
                                                 const char*    key, 
                                                 const char*    value);
 ...
 * @remarks     调用本接口设置变量的值时,并没有将变量值设置入配置表,而是将值进行
                缓冲,只有调用GxBus_ConfigGroupClose()时才会实际设置入配置表
 */
int32_t     GxBus_ConfigGroupSetInt             (handle_t       group, 
                                                 const char*    key, 
                                                 int32_t        value);
 ...
 * @remarks     调用本接口设置变量的值时,并没有将变量值设置入配置表,而是将值进行
                缓冲,只有调用GxBus_ConfigGroupClose()时才会实际设置入配置表
 */
int32_t     GxBus_ConfigGroupSetDouble          (handle_t       group, 
                                                 const char*    key, 
                                                 double         value);
