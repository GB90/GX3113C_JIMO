...
ifndef GXLIB_PATH
	...
	@-rm -f $(GXSRC_PATH)/theme/$(DVB_RESOLUTION)/keymap.xml	
	@cp $(GXLIB_PATH)/include/app/key_xml/$(DVB_KEY_TYPE).xml $(GXSRC_PATH)/theme/$(DVB_RESOLUTION)/keymap.xml
ifeq ($(DVB_LOGO_JPG), )
else
	...
	@cd $(GXSRC_PATH)/theme/$(DVB_RESOLUTION)/image && sh create_image.sh && cd -
	@cd $(GXSRC_PATH)/theme/$(DVB_RESOLUTION)/widget && sh create_widget.sh && cd -
#	@echo "env END...";
...
deps: $(SRC) Makefile
	...
	cd $(GXSRC_PATH); \
	echo "#define DEVELOPMENT_SVN `sed -n -e 11p .svn/entries`"  > $(GXSRC_PATH)/app/include/gxapp_development_svn.h; \
        echo "#define URL_DEVELOPMENT \"`sed -n -e 5p .svn/entries`\"" >> $(GXSRC_PATH)/app/include/gxapp_development_svn.h; \
	cd -; \
	fi;
...
#cth	
	...
	cp -ar ../theme/$(DVB_RESOLUTION) output/theme
...
#	cp $(GXFEKO_PATH) $(NFSROOTFS_PATH)/lib/modules/2.6.27.55/
	...
	cp -ar $(GXSRC_PATH)/theme/$(DVB_RESOLUTION) $(NFSROOTFS_PATH)/dvb/theme
...
ifeq ($(OS), ecos)
	...
	@rm -f ../theme/$(DVB_RESOLUTION)/image/image.xml
	@rm -f ../theme/$(DVB_RESOLUTION)/keymap.xml
	@rm -f ../theme/$(DVB_RESOLUTION)//widget/widget.xml
ifeq ($(OS), linux)
	@rm -rf ../app/output
	@rm -rf ../app/user.bin
