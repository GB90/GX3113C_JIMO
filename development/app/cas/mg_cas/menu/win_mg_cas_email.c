...
void app_mg_cas_email_flag_img_show(uint8_t showFlag)
{
	switch(showFlag)
	{
...
void app_mg_cas_email_list_update()
{
    char* focus_Window = (char*)GUI_GetFocusWindow();
    if(NULL != focus_Window && !strcasecmp("win_mg_cas_email", focus_Window))
    {
        char	 config[10];
...
SIGNAL_HANDLER  int app_mg_cas_email_keypress(const char* widgetname, void *usrdata)
{
	...
	case GUI_KEYDOWN:
		...
		case KEY_EXIT:
		case KEY_RECALL:
			GUI_EndDialog("win_mg_cas_email");
		...
		case KEY_OK:
			...
			{	
				...
				get_data.ID = MG_EMAIL_CONTENT_ID;
				if (NULL != app_cas_api_get_data(&get_data))
				{
					GUI_CreateDialog("win_mg_cas_email_content");
				}
				else
				{
					app_popmsg(POP_DIALOG_X_START, POP_DIALOG_Y_START,"No mail",POPMSG_TYPE_OK);
...
SIGNAL_HANDLER  int app_mg_cas_email_get_data(const char* widgetname, void *usrdata)
{
	...
	{
      if(i==0)
		get_data.ID = MG_EMAIL_FLAG_ID;
      else if(i==1)
        get_data.ID = MG_EMAIL_SENDER_ID;
      else if(i==2)
        get_data.ID = MG_EMAIL_TITLE_ID;
      else if(i==3)
        get_data.ID = MG_EMAIL_SENDTIME_ID;
		...
		case GUI_KEYDOWN:
			switch(event->key.sym)
			{
				...
				case KEY_RECALL:
					get_count.date_type = DVB_CA_EMAIL_INFO;
	    			app_cas_api_get_count(&get_count);
					GUI_EndDialog("win_mg_cas_email_content");
				    return EVENT_TRANSFER_STOP;
                case KEY_UP:
