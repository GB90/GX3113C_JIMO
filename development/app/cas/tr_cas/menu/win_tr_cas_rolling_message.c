...
******************************************************************************
* File Name :	win_tr_cas_rolling_message.c
* Author    : 	zhouhm
...
void tr_cas_rolling_message_show(uint8_t byStyle, uint8_t showFlag, char* message)
{
	switch (byStyle)
	{
		case TR_CA_OSD_TOP:
		{
			...
			else if (DVB_CA_ROLLING_FLAG_CLEAR == showFlag)
			{
				GUI_SetProperty("win_full_screen_text_roll_top","rolling_stop",NULL);										
				GUI_SetProperty("win_full_screen_text_roll_top","reset_rolling"," ");
				GUI_SetProperty("win_full_screen_text_roll_top","string"," ");
		...
		default:
		{
			printf("[tr_cas_rolling_message_show]invalid type(%d)!!!\n", byStyle);
		}
		break;
	}
...
int32_t tr_cas_rolling_message_get_times(uint8_t byStyle)
{
	int32_t wTimes = 0;  
