...
SIGNAL_HANDLER  int win_mg_cas_submaincard_create(const char* widgetname, void *usrdata)
{
 ...
 //   	GUI_SetProperty("win_mgcas_submaincard_tip_image_exit", "img", "tips_exit.bmp");
	...
	get_data.ID = MG312_BASEINFO_SUBMAINCARD_TYPE_ID;
    ...
    get_data.ID = MG312_BASEINFO_SUBMAINCARD_MAINID_ID;
	str = app_cas_api_get_data(&get_data);
	if (NULL != str )
	{
    ...
    get_data.ID = MG312_BASEINFO_SUBMAINCARD_TIME_ID;
	str = app_cas_api_get_data(&get_data);
	if (NULL != str )
	{
	...
	get_data.ID = MG312_BASEINFO_SUBMAINCARD_ACTIVE_ID;
	str = app_cas_api_get_data(&get_data);
	if (NULL != str )
	{
        GUI_SetProperty("win_mg_cas312_submaincard_remainder", "string",str);			
	}
...
SIGNAL_HANDLER  int win_mg_cas_submaincard_keypress(const char* widgetname, void *usrdata)
{
	...
	if(event->type == GUI_KEYDOWN)
	{
		switch(event->key.sym)
		{
			...
			case KEY_RECALL:
				GUI_EndDialog("win_mg_cas312_submaincard");
				return EVENT_TRANSFER_STOP;
			case KEY_OK:
				return EVENT_TRANSFER_STOP;
			case KEY_POWER:
