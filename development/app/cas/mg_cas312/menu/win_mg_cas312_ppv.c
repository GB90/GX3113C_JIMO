...
static int s_ppvCanOrder = FALSE;


int mg312_curprogram_is_ppv(void)
{
    ca_get_date_t get_data = {0};
	char* str = NULL;

	get_data.date_type = DVB_CA_IPPV_POP_INFO;	
	get_data.ID = MG312_PPV_ISNO;
...
//µ±Ç°½ÚÄ¿¶©¹º×´Ì¬
static int mg312_ppv_orderstatus(void)
{
    ca_get_date_t get_data = {0};
	char* str = NULL;

	get_data.date_type = DVB_CA_IPPV_POP_INFO;	
	get_data.ID = MG312_PPV_ORDER_STATUS_ID;
	str = app_cas_api_get_data(&get_data);
	if (NULL != str)
	{	
        return (atoi(str) > 0) ? TRUE : FALSE;
...
static int mg312_ppv_order(int order)
{
    ca_buy_ipp_t data;
...
SIGNAL_HANDLER  int win_mg_ppv_create(const char* widgetname, void *usrdata)
{
	...
	get_data.date_type = DVB_CA_IPPV_POP_INFO;	
	get_data.ID = MG312_PPV_ORDER_STATUS_STRING_ID;
	str = app_cas_api_get_data(&get_data);
	if (NULL != str)
	{
    ...
    get_data.ID = MG312_PPV_PRO_COST_ID;
	str = app_cas_api_get_data(&get_data);
	if (NULL !=str )
	{
    ...
    get_data.ID = MG312_PPV_CARD_BLE;
	str = app_cas_api_get_data(&get_data);
	if (NULL != str)
	{
    ...
    s_ppvCanOrder = (card_Balance >= program_pric)?TRUE:FALSE;
    
    if (FALSE == mg312_ppv_orderstatus())
    {
...
SIGNAL_HANDLER  int win_mg_ppv_keypress(const char* widgetname, void *usrdata)
{
	...
	if (event->type == GUI_KEYDOWN)
	{
		...
		{
			case KEY_EXIT:
                GUI_EndDialog("win_mg_cas312_ppv"); 
				break;
			...
			case KEY_OK:
            {
                ...
                if (0 == strcmp(name, "win_mg_ppv_button_yes"))
    			{
    				if (FALSE == mg312_ppv_orderstatus())
                    {
                        if (TRUE == s_ppvCanOrder)
                        {
                            mg312_ppv_order(TRUE);
                            printf("mg312 ppv todo order!\n");
                        }
                        else //Óà¶î²»×ã
                        {
    			...
    			{
    				if (TRUE == mg312_ppv_orderstatus())
                    {
                        mg312_ppv_order(FALSE);
                        printf("mg312 ppv todo unorder!\n");
                    }
    			}
		    }
