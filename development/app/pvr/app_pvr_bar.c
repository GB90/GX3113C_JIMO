...
/*static*/ void full_pause_exec(void)
{
    ...
    else if(g_AppPvrOps.usb_check(&g_AppPvrOps) == USB_ERROR)
    {
            //notic usb error
		#ifdef APP_SD
		app_popmsg(210,150,"Insert the device please",POPMSG_TYPE_OK);
		#endif
		#ifdef APP_HD
		app_popmsg(340,200,"Insert the device please",POPMSG_TYPE_OK);
		#endif
            return;
    }
    else if(g_AppPvrOps.usb_check(&g_AppPvrOps) == USB_NO_SPACE)
    {
            //notice usb no space
		#ifdef APP_SD
		app_popmsg(210,150,"No space!",POPMSG_TYPE_OK);
		#endif
		#ifdef APP_HD
		app_popmsg(340,200,"No space!",POPMSG_TYPE_OK);
		#endif
            return;
    }
    else
...
static void pvr_cur_total_tick_get(void)
{
...
//    AppPvrUi *pvr_ui = &g_AppPvrUi;
    ...
    if(GXCORE_SUCCESS  == GxPlayer_MediaGetTime((char*)(time.player_name), &(time.cur_time), &(time.total_time)))
    {
...
static void pvr_name_show(void)
{
	...
	else
	{
		...
		{
			sprintf(prog_name,"%d-%d %s  %d-%d(正在录制...)", 
             (prog_data.pos & 0xFF00)>>8, (prog_data.pos & 0xFF),prog_data.prog_name,
			(prog_data_pvr.pos & 0xFF00)>>8,(prog_data_pvr.pos & 0xFF));
		}
		else
		{
			sprintf(prog_name,"%d-%d %s  %d-%d(Recording...)", 
             (prog_data.pos & 0xFF00)>>8, (prog_data.pos & 0xFF),prog_data.prog_name,
			(prog_data_pvr.pos & 0xFF00)>>8,(prog_data_pvr.pos & 0xFF));
		}
...
uint8_t app_pvr_state_check(uint8_t flag, uint32_t pos)
{
	...
	if(flag == 0)
	{
		if(pvr->state == PVR_TIMESHIFT||pvr->state == PVR_TMS_AND_REC)
		{
			//notice
			#ifdef APP_SD
			app_popmsg(210,150,"Timeshift...",POPMSG_TYPE_OK);
			#endif
			#ifdef APP_HD
			app_popmsg(340,200,"Timeshift...",POPMSG_TYPE_OK);
			#endif
			return 1;
		}
		if(pvr->state == PVR_RECORD)
		{
			#ifdef APP_SD
			app_popmsg(210,150,"Recording...",POPMSG_TYPE_OK);
			#endif
			#ifdef APP_HD
			app_popmsg(340,200,"Recording...",POPMSG_TYPE_OK);
			#endif
			return 1;
		}
	}
	else if(flag == 1)
	{
		if(pvr->state == PVR_TIMESHIFT||pvr->state == PVR_TMS_AND_REC)
		{
			//notice
			#ifdef APP_SD
			if(POPMSG_RET_YES == app_popmsg(210,150,"Stop Timeshifting?",POPMSG_TYPE_YESNO))
			#endif
			#ifdef APP_HD
			if(POPMSG_RET_YES == app_popmsg(340,200,"Stop Timeshifting?",POPMSG_TYPE_YESNO))
			#endif
			{
				app_pvr_stop();
				return 0;
		...
		if(pvr->state == PVR_RECORD)
		{
			...
			else
			{
				//notice
				#ifdef APP_SD
				if(POPMSG_RET_YES == app_popmsg(210,150,"Stop recording?",POPMSG_TYPE_YESNO))
				#endif
				#ifdef APP_HD
				if(POPMSG_RET_YES == app_popmsg(340,200,"Stop recording?",POPMSG_TYPE_YESNO))
				#endif
				{
					app_pvr_stop();
					return 0;
    ...
    switch(state)
    {
        case PLAYER_STATUS_RECORD_FULL:
        {
		   ...
		   {   
            ...
            extern void set_win_pop_tip_autoclear(int clearFlag);
            extern int win_pop_tip_show_hotplug_gxmsg(char* msg);
            set_win_pop_tip_autoclear(0);
...
#endif
						#ifdef APP_SD
                        if(skip_popmsg || POPMSG_RET_YES == app_popmsg(210,150,str,POPMSG_TYPE_YESNO))
						#endif
						#ifdef APP_HD
						if(skip_popmsg || POPMSG_RET_YES == app_popmsg(340,200,str,POPMSG_TYPE_YESNO))
						#endif
                        {
				if(gEventState.pause ==  STATE_ON)
				{
