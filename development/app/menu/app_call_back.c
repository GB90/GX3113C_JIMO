	...
	*/
	...
	{	
        ...
        if(g_AppPvrOps.state != PVR_DUMMY)
    	{
            ...
            extern void event_to_pvr(uint8_t flag) ;
            event_to_pvr(2);
    	}
#endif    
...
/*		if (searchExtendList->extendnum >=SEARCH_EXTEND_MAX)
		...
		searchExtendList->searchExtend[searchExtendList.extendnum].pid = SDT_PID;
		searchExtendList->searchExtend[searchExtendList.extendnum].match_depth = 1;
		searchExtendList->searchExtend[searchExtendList.extendnum].match[0] = SDT_ACTUAL_TS_TID;
		searchExtendList->searchExtend[searchExtendList.extendnum].mask[0] = 0xff;
		searchExtendList->searchExtend[searchExtendList.extendnum].table_parse_fun = sdt_descriptor_parse;	
			...
			if(MAIN_MENU_WIN == app_win_get_focus_video_window()) 
			{
				...
				if(WEEKLY_EPG_WIN == app_win_get_focus_video_window()) 
				{
					...
					if(PROGRAM_EDIT_WIN == app_win_get_focus_video_window()) 
					{
						...
						if(FULL_SCREEN_WIN == app_win_get_focus_video_window()) 
						{
							GUI_SetProperty("win_full_screen_text_msg","state","osd_trans_hide");
						//	GUI_SetProperty("win_full_screen_image562","state","hide");					
						//	GUI_SetProperty("win_full_screen_image563","state","hide");					
...
void app_win_prompt_msg_exec_call_back(void)
{
	...
	else
	{
		...
		* 提示无节目
		*/
		//20141212,qm, 会出现死循环,app_play_set_msg_pop_type会回调当前函数
		#if 0
		//app_play_set_msg_pop_type(MSG_POP_NO_PROG);
        #else
