				...
				switch(event->key)
					{
						...
						case DVB_CA_OTA:
							...
							ret = app_popmsg(POP_DIALOG_X_START, POP_DIALOG_Y_START,"A new software,whether to upgrade",POPMSG_TYPE_YESNO);
						...
						case DVB_CA_CONDITION_SEARCH:
							...
							break;
#endif
						case DVB_CA_CHANGE_INFO:
							TOP_Msg_Dbg("CA卡改变,需要重启\n");
							int flag = 0;
							char caStrbuf[100] = {0,};
							flag = *(int*)event->buf;
							printf("ca change %d \n",flag);
							app_set_popmsg_yes_timeout(3);
							if( flag == 0 )
							{
								sprintf(caStrbuf,"%s","腾锐ca，等待重启！")	;
							}
							else
							{
								sprintf(caStrbuf,"%s","德芯ca，等待重启！")	;
							}	
							ret = app_popmsg(POP_DIALOG_X_START, POP_DIALOG_Y_START,caStrbuf,POPMSG_TYPE_OK);
							GxCore_ThreadDelay(500);
							restart();
							break;
						default:
