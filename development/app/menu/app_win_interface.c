...
{
    printf("[%s] set the msp index as MESSAGE_MAX_COUNT.\n", __FUNCTION__);
							...
							if (NULL != focus_Window)
								{
									...
									if ((0 == strcasecmp("win_full_screen" ,widget_win_name))&&
										((0 == strcasecmp("win_sys_set" ,focus_Window))||(0 == strcasecmp("win_media_centre" ,focus_Window))))
										{
											/*
...
int app_win_check_fre_vaild(FRE_TYPE_E type,uint32_t fre)
{
    ...
    if(FRE_MID == type)
    {
	    if (fre < FRE_BEGIN_LOW)
		{
			...
			{
				sprintf((void*)buf,"错误频点，频点应大于等于 %03d", 
						FRE_BEGIN_LOW);
			}
			else
			{
				sprintf((void*)buf,"Fre para error , should not less than %03d", 
						FRE_BEGIN_LOW);
			}
		...
		if (fre > FRE_BEGIN_HIGH)
		{
			...
			if (0 == strcmp(osd_language,LANGUAGE_CHINESE))
			{
				sprintf((void*)buf,"错误频点，频点应小于等于 %03d", 
						FRE_BEGIN_HIGH);
			}
			else
			{
				sprintf((void*)buf,"Fre para error , should not larger than %03d", 
						FRE_BEGIN_HIGH);
			}
			retResult = app_popmsg(POP_DIALOG_X_START, POP_DIALOG_Y_START,buf, POPMSG_TYPE_OK);
    ...
    else if(FRE_BEGIN == type)
    {
	    if (fre < FRE_BEGIN_LOW)
		{
			...
			if (0 == strcmp(osd_language,LANGUAGE_CHINESE))
			{
				sprintf((void*)buf,"错误频点，开始频点应大于等于 %03d", 
						FRE_BEGIN_LOW);
			}
			else
			{
		...
		if (fre > FRE_BEGIN_HIGH)
		{
			...
			if (0 == strcmp(osd_language,LANGUAGE_CHINESE))
			{
				sprintf((void*)buf,"错误频点，开始频点应小于 %03d", 
						FRE_BEGIN_HIGH);
			}
			else
			{
    ...
    else if(FRE_END == type)
    {
	    if (fre < FRE_BEGIN_LOW)
		{
			...
			if (0 == strcmp(osd_language,LANGUAGE_CHINESE))
			{
				sprintf((void*)buf,"错误频点，结束频点应大于 %03d", 
						FRE_BEGIN_LOW);
			}
			else
			{
		...
		if (fre > FRE_BEGIN_HIGH)
		{
			...
			if (0 == strcmp(osd_language,LANGUAGE_CHINESE))
			{
				sprintf((void*)buf,"错误频点，结束频点应小于等于 %03d", 
						FRE_BEGIN_HIGH);
			}
			else
			{
...
int app_win_auto_search(void)
{
	...
	app_search_scan_all_mode(FRE_BEGIN_LOW_FULL,FRE_BEGIN_HIGH_FULL,symbol_rate,qam);
#else
...
int app_win_reset_factory(void)
{
		...
		GxBus_ConfigSetInt (VIDEO_COVER_FRAME , 9);
#endif
