...
SIGNAL_HANDLER int pic_view_init(const char* widgetname, void *usrdata)
{
	...
	*/	
	...
	app_set_win_create_flag(MEDIA_PIC_WIN);
	//打开硬解
	GDI_RegisterJPEG();
...
SIGNAL_HANDLER int pic_view_destroy(const char* widgetname, void *usrdata)
{
 ...
 GUI_SetInterface("free_space", "fragment|back_osd");//
		...
		/*
		* refresh file browser select item
		*/
		play_pic_get_info(&info);
		for (sel = 1; sel<=explorer_view->nents;sel++)
			{
				ent = explorer_view->ents + (sel - 1);
				if(NULL == ent)
					{
						break;
					}
				...
				if(GX_FILE_REGULAR == ent->ftype)
					{
						if(strcmp(info.name, ent->fname) == 0)
							{
		...
		app_set_win_destroy_flag(MEDIA_PIC_WIN);
		//恢复软解
		GDI_UnRegisterJPEG();
#endif
