...
#ifdef CUST_YUEHAI
GUI_SetProperty(OSDSET_COMBOBOX_MENU_LANGUAGE, "content",
				"[Chinese,English,WEIWEN]");
...
#ifdef CUST_YUEHAI
    if (0 == strcmp(osd_language,LANGUAGE_WEIWEN))
	    widget_value = 2;
#endif
...
static int set_menu_language(int widget_value, int flag_set, int flag_save)
{
...
#else
	    else if( flash_value ==1 )
	    {
		    app_flash_save_config_osd_language(LANGUAGE_ENGLISH);				
	    }
	    else
	    {
		    app_flash_save_config_osd_language(LANGUAGE_WEIWEN);				
	    }
    ...
    if (2 == flash_value)
	    app_play_set_osd_language(LANGUAGE_WEIWEN);				
#endif
    return 0;
}
