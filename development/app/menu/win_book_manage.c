...
static int timer_book(void *userdata)
{
	...
	uint32_t sel = 0;
	app_book_sync(&EpgBookGet, BOOK_PROGRAM_PLAY);
...
SIGNAL_HANDLER int book_manage_create(const char* widgetname, void *usrdata)
{
    ...
    {
//  	GUI_SetProperty("win_book_manage_title", "img", "title_bookmanager.bmp");

  //  	GUI_SetProperty("win_book_manage_tip_image_exit", "img", "tips_exit.bmp");
    }
    else 
    {
//    	GUI_SetProperty("win_book_manage_title", "img", "title_bookmanager_e.bmp");
  //  	GUI_SetProperty("win_book_manage_tip_image_exit", "img", "tips_exit_e.bmp");
     }
...
SIGNAL_HANDLER int book_manage_keypress(const char* widgetname, void *usrdata)
{
	...
	switch(event->key.sym)
	{
		...
		case KEY_RED:
			...
			{
				GUI_GetProperty(BOOK_LISTVIEW, "select", &sel);	
				if(-1 == app_book_check_exist(&pEpgBookGet,BOOK_PROGRAM_PLAY|BOOK_TYPE_1,EpgBookGet.book[sel].id))
				{
					return EVENT_TRANSFER_STOP;
				}
				app_book_delete(&EpgBookGet.book[sel]);
