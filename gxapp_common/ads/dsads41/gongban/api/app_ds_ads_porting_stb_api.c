...
static EN_DSAD_OSD_SHOW_POS osd_showing_type = DSAD_SHOW_UNKNOWN_TYPE;
static BOOL new_osd_flag = FALSE;
...
extern ST_DSAD_OSD_PROGRAM_SHOW_INFO osd_ProgramShowInfo;
static handle_t ds_ad_osd_sem ;
void ds_ad_rolling_set_osd_flag(char flag)
{
	new_osd_flag =flag;
}
BOOL ds_ad_rolling_get_osd_flag(void)
{
	return new_osd_flag;
}
...
static void app_ds_ad_flash_init(void)
{
	...
	memset(&osd_ProgramShowInfo,0,sizeof(ST_DSAD_PROGRAM_SHOW_INFO));
...
static void do_osd_show(ST_DSAD_OSD_PROGRAM_SHOW_INFO * osd_info,uint8_t curIndex)
{
	...
	printf("TRACK %s %d IN curIndex=%d\n",__FUNCTION__,__LINE__,curIndex);
	if(osd_info == NULL)
	{
		return;
	}
	...
	for(i = 0;i < DSAD_OSD_DISPLAY_MAX_NUMBER ;i++)
	{
	...
	if(get_osd_showing_top_status()&&0==strcmp(ad_osd->pucDisplayBuffer,get_osd_showing_buf_top()))
	{
		return;
	}
	if(get_osd_showing_bottom_status()&&0==strcmp(ad_osd->pucDisplayBuffer,get_osd_showing_buf_bottom()))
	{
		return;
	}
	#if 0
	if(DSAD_SHOW_UNKNOWN_TYPE == osd_showing_type)
	{
		app_ds_ad_hide_osd();
	}
	...
	data.ucOSDTempo = ad_osd->ucOSDTempo;;//osd滚动速度
#endif	
...
static uint8_t app_ds_ad_show_pic(DS_AD_PIC_OPT opt,char * IconName,EN_DSAD_POS_TYPE adType)
{
	...
	if(opt == LOAD_PIC_DATA)/*load*/
	{
		...
		if(ret == DSAD_FUN_OK)
		{
			...
			ptrPic->ptrTimer = create_timer((timer_event)app_ds_ad_show_gif_callback,20,ptrPic,TIMER_REPEAT);
			bRet = 1;
		}
		else
		{
...
//	printf("TRACK %s %d IN\n",__FUNCTION__,__LINE__);
	...
	do{
...
#else
		memcpy(&osd_info,&osd_ProgramShowInfo,sizeof(ST_DSAD_OSD_PROGRAM_SHOW_INFO));
#endif
		maxIndex = osd_info.ucOsdNum;
		/*OSD is not showing OR programe is changed*/
		//		ADS_Dbg("maxIndex = %d  lastServiceId=0x%x  service_id =0x%x\n",maxIndex,lastServiceId,prog.service_id);
		if(TRUE==ds_ad_rolling_get_osd_flag()&&((lastServiceId != prog.service_id)||(DSAD_SHOW_UNKNOWN_TYPE==osd_showing_type)))
 		{
		...
		//		ADS_Dbg("original_id=%d,ts_id=%d,service_id=%d\n",prog.original_id,prog.ts_id,prog.service_id);
				...
				if ((ret == DSAD_FUN_OK) && (maxIndex != 0) )
...
#endif
					{
						...
						ADS_Dbg("maxIndex = %d  lastServiceId=0x%x  DSAS_FUN_OK\n",maxIndex,lastServiceId);
						lastServiceId = prog.service_id;
		...
		/*OSD is showing*/
		if ((osd_showing_type!=DSAD_SHOW_UNKNOWN_TYPE) && app_desai_cas50_rolling_message_one_time())
		{
			if(0==maxIndex)//NO OSD SO HIDE 
			{
				maxIndex = 0;
				lastIndex = 0;
				lastServiceId = prog.service_id;
...
#ifdef DVB_AD_TYPE_DS_FLAG41
				DSAD_SendShowOSDNum(lastIndex);
				//ADS_Dbg("\n 1 DSAD_SendShowOSDNum(%d)\n",lastIndex);
...
#ifdef DVB_AD_TYPE_DS_FLAG41
void app_ds_ad_sendprogram(uint16_t networkId,uint16_t tsId,uint16_t serviceId)
{
	static uint16_t serviceId_temp = 0xFFFF;
	static uint16_t networkId_temp = 0xFFFF;
	static uint16_t tsId_temp = 0xFFFF;
	ST_DSAD_PROGRAM_PARA sProgramPara = {0,};
	memset(&sProgramPara,0,sizeof(sProgramPara));
	sProgramPara.usNetworkId = networkId;
	sProgramPara.usTsId = tsId;
	sProgramPara.usServiceId = serviceId;
	if (( serviceId!=serviceId_temp)||(networkId != networkId_temp)||(tsId!=tsId_temp) )
	{
		app_ds_ad_hide_osd();
		DSAD_SendShowOSDNum(0);	
		serviceId_temp = serviceId;
		networkId_temp = networkId;
		tsId_temp =  tsId;
	}
	ADS_Dbg("\n DSAD_SendShowOSDNum(0)\n");
	DSAD_SendProgram(sProgramPara);
