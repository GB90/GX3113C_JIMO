...
ST_DSAD_OSD_PROGRAM_SHOW_INFO osd_ProgramShowInfo;
static uint32_t		DSAD_DELECMD_CRC = 0;
static uint32_t		DSAD_CONFIG_CRC = 0;
...
EN_DSAD_FUNCTION_TYPE DS_AD_SetFilter(ST_DSAD_FILTER_INFO *pstFilterInfo)
{
	...
	if(get_porting_query_demux_runflag())
	{
		return DSAD_FUN_ERR_PARA;
	}
...
#endif
	//CAS_DUMP("DS_AD_SetFilter pbyFilter", pstFilterInfo->aucFilter , pstFilterInfo->ucFilterLen);
	//CAS_DUMP("DS_AD_SetFilter pbyMask", pstFilterInfo->aucMask , pstFilterInfo->ucFilterLen);
...
void DSAD_ShowMessage(UINT8 ucMessageType, IN UINT8 * pucMessage)
{
	...
	ST_DSAD_OSD_PROGRAM_SHOW_INFO *pstProgramShowInfo = NULL;
	...
	switch(ucMessageType)
	{
		...
		pstProgramShowInfo = (ST_DSAD_OSD_PROGRAM_SHOW_INFO *)pucMessage;
		if (pstProgramShowInfo == NULL)
		{
			break;
		}
		memset(&osd_ProgramShowInfo,0,sizeof(ST_DSAD_OSD_PROGRAM_SHOW_INFO));
		memcpy(&osd_ProgramShowInfo,pstProgramShowInfo,sizeof(ST_DSAD_OSD_PROGRAM_SHOW_INFO));
		ds_ad_rolling_set_osd_flag(TRUE);
		printf("osd_programshowinfo usosdnum = %d \n",osd_ProgramShowInfo.ucOsdNum);
#if 0
		for( i=0 ; i<DSAD_OSD_DISPLAY_MAX_NUMBER ; i++ )
		{
		printf("osd_programshowinfo flag[%d] = %d \n",i,osd_ProgramShowInfo.aucFlag[i]);
		}
#endif		
		break;
#endif
