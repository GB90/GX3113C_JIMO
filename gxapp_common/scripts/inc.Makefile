...
ifeq ($(debug),yes)
CFLAGS += -pipe -O0 -g -DMEMORY_DEBUG
else
CFLAGS += -pipe -O2 -g 
endif
...
conf:
	...
	echo '#define RC_VERSION $(RC_VERSION)'>> include/gxapp_sys_config.h
ifeq ($(OS), linux)
...
ifeq  ($(DVB_AUTO_TEST_FLAG), yes)
	...
	if [ -f .svn/entries ] ; then \
                echo "#define APP_COMMON_SVN `sed -n -e 11p .svn/entries`" >> include/gxapp_sys_config.h; \
                echo "#define URL_GXAPP_COMMON \"`sed -n -e 5p .svn/entries`\"" >> include/gxapp_sys_config.h; \
   	 fi;
