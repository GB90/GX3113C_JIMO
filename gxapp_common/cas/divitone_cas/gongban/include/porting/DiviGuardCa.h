/********************************************************************/
/*	深圳市数视通科技股份有限公司 CopyRight 1998-2011            */
/*	数字电视条件接收系统终端库 Ver2.0                             */
/*	机顶盒移植库头文件                                          */
/*	版本：2.1.0.0                                               */
/*	作者：                                               */
/*	日期：2007-9-3                                              */
...
typedef struct _TotalChildInfo{
     ...
     UINT8 Main_Card_NO[8];
     UINT8 Active_Duration[6];	 
     UINT8 Effective_Days;
     UINT8 Lead_Time;
     UINT8 Temp_Active_Duration;	   
...
输入参数： 
		bType： 保留无用；
              pEcmInfo:T_CAServiceInfo结构，描述节目的详细信息; 
                            若pEcmInfo->m_wEcmPid为0xffff,则释放channel和filter。
...
输入参数： 
		bType，保留无用； 
              wEmmPid，对应于DIVI 的CAS_System_ID的EMM的PID值；
                                 若wEmmPid为0xffff,则释放channel和filter。 
...
FAILTURE： 不是CA的智能卡。 
*/
extern INT32  DIVI_SC_ValidateATR(UINT8 * pbNewATR,UINT16 wLen);
...
/*提供者：机顶盒
使用者：CA模块
功能描述：获得机顶盒提供给CA模块用于保存机卡配对信息的起始地址和空间大小；空间
大小最少为16个字节
输入参数：
输出参数：
...
输入参数： 无
输出参数： 无 
返 回 值： 0x01---机卡配对;0x02 --- 未机卡配对,0x00 ---未插卡*/
...
/*
INT32 UnlockPIN(void)
提供者:CA模块
使用者:机顶盒
功能描述:解锁Pin码, PIN码恢复为888888 
输入参数:无  
输出参数:无
返回值:
		SUCCESS---解锁成功
        FAILTURE---解锁失败
*/
//INT32 UnlockPIN(void);//danlly, 2010-05-20

/*
INT32 DIVI_ClearPaired(void)
提供者:CA模块
使用者:机顶盒
功能描述:清除机卡配对标识
输入参数:无  
...
返回值:
        ...
        DIVI_ERR---清除失败
*/
INT32 DIVI_ClearPaired(void);//danlly, 2010-05-20
