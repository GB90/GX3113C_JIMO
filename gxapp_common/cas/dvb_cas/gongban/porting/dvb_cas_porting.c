...
int tnt_smartcard_softca_transfer(unsigned char *Cmd,
                            unsigned short CmdL,
                            unsigned char *Resp,
                            unsigned short *RespL,
                            unsigned char *StatusWords)

{
    int  len;
#define SMC_MAX_REPLY			(256)
#define	SMC_MAX_CMD_LEN			(256)
    unsigned char cmd[SMC_MAX_CMD_LEN];
	unsigned char reply[SMC_MAX_REPLY];
	unsigned char *pSW1 = StatusWords;
	unsigned char *pSW2 = (StatusWords + 1);
	unsigned short CmdLen = CmdL;
	unsigned int   rep_size;
	unsigned short *RespLen = RespL;
	unsigned char *pResponse= Resp;
    ...
    *pSW1 = *pSW2 = 0;
	if(CmdLen <=5 )
	{
		len = PlatformSendApduToCard(cmd,CmdLen);
		if (len == CmdLen)
		{
			len = PlatformGetApduFromCard(reply,&rep_size);
//			CA_DEBUG("cmd reply len[%d],rep_size[%d]\n",len,rep_size);
			//TNT_SHOW_ARRAY(reply,rep_size);
	...
	{
		len = PlatformSendApduToCard(cmd,5);
		if (len == 5)
		{
			len = PlatformGetApduFromCard(reply,&rep_size);
			//CA_DEBUG("cmd reply len[%d],rep_size[%d]\n",len,rep_size);
		//	TNT_SHOW_ARRAY(reply,rep_size);
			if(cmd[1] == reply[0])
			{
				len = PlatformSendApduToCard(cmd + 5,CmdLen - 5);
				len = PlatformGetApduFromCard(reply,&rep_size);
			//	CA_DEBUG("cmd reply len[%d],rep_size[%d]\n",len,rep_size);
		//		TNT_SHOW_ARRAY(reply,rep_size);
				if(len > 0)
	...
	unsigned char*      pucStatusWord)
{
	...
	return DVBCA_FAILED;
#else
    int ret = DVBCA_SUCESS;
  //  TNT_DEBUG("\nCASCAM, TNT CAS, %s, %d\n", __FUNCTION__,__LINE__);
    //ret = tnt_smartcard_transfer(pucCommand, usCommandLen, pucReply, pusReplyLen, pucStatusWord);
    ret = tnt_smartcard_softca_transfer(pucCommand, usCommandLen, pucReply, pusReplyLen, pucStatusWord);
    if(ret < 0)
    {
        //ret = DVBCA_FAILED;
    }
...
void DVBSTBCA_UpgradeFreq(ST_DVBCA_FREQ *pstFreq, unsigned char ucFreqSum)
{
	...
	app_win_dvb_upgrade_freq(pstFreq, freqSum);
