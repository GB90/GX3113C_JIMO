...
uint16_t app_qilian_cas_api_get_emm_pid(void)
{
...
//							sfilter->nms = 0;
			...
			STBCA_StreamGuardDataGot(byReqID,TRUE,pid,data,section_length);
			//for bat
			if(pid==0x11)
			{
				printf("qilian ca for bat\n");
				bat_recieve_flag = TRUE;
			}
				...
				if (Filter == filter.handle)
					{
						...
						/* 私有数据接收回调 */
					//STBCA_StreamGuardDataGot	BYCASTB_StreamGuardDataGot( filter->byReqID,FALSE,pid,NULL,0); 		
//STBCA_StreamGuardDataGot(BU8 ReqID,BU8 bSuccess,BU16 wPID,BU8* b8ReceiveData,BU32 wLen);
						STBCA_StreamGuardDataGot( filter.byReqID,FALSE,pid,NULL,0); 
					}												
			}
	}
...
uint8_t	app_qilian_cas_api_release_ecm_filter(void)
{
	...
	*/
	...
	STBCA_SetCurECMInfo(0X1FFF,0X1FFF);
	...
	if (0x1FFF != playPara.p_ecm_pid)
		{
				...
				STBCA_SetCurECMInfo(playPara.p_ecm_pid,playPara.program_num);
		...
		if (0x1FFF != playPara.p_ecm_pid_audio)
		{
				...
				STBCA_SetCurECMInfo(playPara.p_ecm_pid_audio,playPara.program_num);
		}
	...
	if (0x1FFF != playPara.p_ecm_pid_video)
		{
				...
				STBCA_SetCurECMInfo(playPara.p_ecm_pid_video,playPara.program_num);
		}
...
{
	printf("app_qilian_cas_api_nit_zone_des STBCA_CurrentNitDes\n");
	if(TRUE == bat_recieve_flag)
		return ;
	return STBCA_CurrentNitDes(databuf);
}
