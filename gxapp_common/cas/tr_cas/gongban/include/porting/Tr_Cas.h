...
/*wangjian add on 20141227*/
#define TRCAS_MIN(a, b)   (((a) <= (b)) ? (a) : (b))
#define TRCAS_MAX(a, b)   (((a) > (b)) ? (a) : (b))

#define TRCAS_ENTITLE_NUM_MAX   (320)
#define TRCAS_TITLE_LEN  	  	(MSG_TITLE_MAX_LENGTH)
#define TRCAS_CONTENT_LEN      	(MSG_DATA_MAX_LENGTH)

/*wangjian add on 20150105*/
#define TRCAS_PIN_MAXLEN    	(6) 

/*wangjian add on 20150112*/
typedef S16 (*TRCAS_SC_ATR_NOTIFY)(U8 bCardNumber, U8 bStatus, U8 *pbATR, U8 bProtocol);
/*wangjian add end.*/
...
typedef enum
{
	CAS_MC_DECODER_INFO,            /* It is for dual CA communication. */
...
typedef enum
{
    ...
    CAS_STATE_E13,  // Dual communication error
	...
	CAS_STATE_INFO_MAX = CAS_STATE_E36 /*wangjian add*/
} CAS_STATE_INFO;
...
#if 0 /*wangjian modify on 20141231*/
typedef struct
{
    U16 wTitleLen;
    U8  bMsgTitle[80];
...
typedef struct
{
    ...
    U8  bClass;
    U8  bPriority;
    U16 wPeriod;    /* Notification displaying interval. The unit is second. */
    CAS_TIMESTAMP sMsgTime;
    CAS_TIMESTAMP sCreateTime;
} CAS_MSG_STRUCT;
