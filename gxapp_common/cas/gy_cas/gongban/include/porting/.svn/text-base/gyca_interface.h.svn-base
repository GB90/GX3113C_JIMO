...
**	使 用 者：  	机顶盒。
**	功能描述：	获取用户的PIN码。
**	输入参数：	pbPin，PIN码，必须为6个字节的指针。
**	输出参数： pbPin，PIN码，必须为6个字节的指针。
**	返 回 值：  	GYCA_ERR, 未能成功更改。
**					GYCA_OK，成功更改。
**	其他说明：
*************************************************************************************************/
extern GYS32 GYCA_GetPin(GYU8 *pbPin);
...
**     提 供 者：   CA模块。 
**     使 用 者：   机顶盒。 
**     功能描述：   供机顶盒选择提示消息的语言类型。中文和英文。
**     输入参数：   byLanguage - 0为中文，1为英文，其他值无效。默认为0中文。
**     输出参数：   无。 
**     返 回 值：   无
**     其他说明：   
***********************************************************************************/
void   GYCA_SelectLanguage(GYU8 byLanguage); 
...
**     输入参数:	pbPin    机顶盒智能卡的PIN码,6个字节
					...
					endTime智能卡解密结束时间
**     输出参数:	无
**     返 回 值: 	GYCA_OK：成功；GYCA_ERR： 发生错误。 
**     其他说明:	时间为24小时时间格式，如08:01:05~23:45:23。 
					如果起始时间小于结束时间，则工作时间段为当天；
					如果起始时间大于结束时间，则工作时间段为当天起始时间到次日结束时间；
					如果起始时间等于结束时间，则工作时间段为全天；
***********************************************************************************/
extern GYS32 GYCA_SetWorkTime(const GYU8 *pbPin, T_CA_Time *startTime, T_CA_Time *endTime); 
...
**     功能描述:	取得CA解密工作时间段
**     输入参数:	无
**     输出参数:	startTime智能卡解密开始时间
					endTime智能卡解密结束时间
**     返 回 值: 	GYCA_OK：成功；GYCA_ERR： 发生错误。 
**     其他说明:     ------------------------------ <铁岭版本专用>
***********************************************************************************/
extern GYS32 GYCA_GetWorkTime(T_CA_Time *startTime, T_CA_Time *endTime); 


/**********************************************************************************
**     提 供 者：   CA模块。 
**     使 用 者：   机顶盒。 
**     功能描述：   机顶盒把营运商指定bat表中的私有描述子提供给CAM。
**     输入参数：    ts_id 频点传输流ID。 
              desc_buf 描述信息缓冲区地址
              desc_len 描述信息缓冲区长度
              index    SetEntitlemetInfo的调用计数, 如果是重新搜索BAT表后重新
                       调用, 则index从0 开始重新为每个ts流的描述子做一次调用
                       Index每次加1
              bLastTs  取1时表示是否是最后一个流的节目产品信息描述信息
                       否则取0
**     输出参数：   无。 
**     返 回 值：   0 成功 -1 失败 
**     其他说明：   营运商在每个频点提供了关于本频点节目所在CA产品包的描述信息
CA模块需要取得这些描述信息,来判断哪些节目具有CA授权.

备注:               ------------------------------ <铁岭版本专用>
铁岭的主频点bouquet_id 为 0x0x7012的BAT表中 tag=0x90的私有描述子是节目的CA产品信息表, 基本包的编号为1, 其余为增值业务包;(参考后面的附图)
描述子位于BAT表第二循环(transpoer_stream_loop), 每个传输流都可能存在该描述子
即使没有过滤到该描述子, 也需要调用该函数, desc_len被设置为0
,
铁岭的主频点bouquet_id 为 0x0x7011的BAT表中 tag=0x90的私有描述子是节目排序规则表;
***********************************************************************************/
extern GYS32  GYCA_SetEntitlemetInfo(GYS32 ts_id, GYU8* desc_buf, GYS32 desc_len, int index, GYS32  bLastTsId); 


/**********************************************************************************
**     提 供 者：   	CA模块。 
**     使 用 者：   	机顶盒。 
**     功能描述：   	判断该节目是否在节目列表中不被隐藏。
**     输入参数：   	ts_id 频点传输流ID
						Srv_id 节目service_id。 
**     输出参数：   	无。 
**     返 回 值：   	0 节目在节目列表中被隐藏
...
**     其他说明：        ------------------------------ <铁岭版本专用>
***********************************************************************************/
extern GYS32  GYCA_GetEntitlementFlag(GYS32 ts_id, GYS32 srv_id); 
