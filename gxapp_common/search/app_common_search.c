...
//	app_lcn_list_clear();
	...
	*/
	...
	app_lcn_list_init();
#ifdef CA_FLAG
#if (DVB_DEMOD_DVBC == DVB_DEMOD_MODE) &&(RC_VERSION>GOXCEED_v1_9_2)
...
//	app_pmt_close();
	...
	sat.tuner = 0;
	sat.sat_dtmb.work_mode = app_flash_get_config_gx150x_mode_flag();
	GxBus_PmSatAdd(&sat);
	GxBus_PmSync(GXBUS_PM_SYNC_SAT);
#endif
...
*end_fre -- 结束频率
*/
uint32_t app_search_scan_all_mode(uint16_t begin_fre,uint16_t end_fre,uint32_t symbol_rate,uint32_t qam)
{
    uint16_t i;
    uint16_t fre;
    uint16_t freArrayCount =0;
    uint16_t end_fre_tmp = end_fre;
...
//特定频率搜索
uint32_t app_search_scan_mode(uint16_t *fre,uint16_t size,uint32_t symbol_rate,uint32_t qam)
{
    uint16_t i;
...
{
	search_fre_list t_searchFreList={{474,626,706,714,730},{2,2,2,2,2},{6875,6875,6875,6875,6875},{0,0,0,0,0},5,1};
...
{
	search_fre_list t_searchFreList={{746,762,770,794,802},{2,2,2,2,2},{6875,6875,6875,6875,6875},{0,0,0,0,0},5,1};
	GUI_CreateDialog("win_search_result");
	GUI_SetInterface("flush", NULL);
	app_search_enum_enum_mode(t_searchFreList);
	return EVENT_TRANSFER_KEEPON;	
...
{
	search_fre_list t_searchFreList={{474,626,706,714,730,746,762,770,794,802},{2,2,2,2,2,2,2,2,2,2},{6875,6875,6875,6875,6875,6875,6875,6875,6875,6875},\
...
#ifdef CUST_JIMO
int app_win_jimo_search(void)
{
	search_fre_list t_searchFreList={{522,610,714},{2,2,2},{6875,6875,6875},\
				{0,0,0},3,1};
	app_search_enum_enum_mode(t_searchFreList);
	return EVENT_TRANSFER_KEEPON;	
}
...
extern popmsg_ret app_popmsg(uint32_t x, uint32_t y, const char* context, popmsg_type type);
...
void app_win_dvb_upgrade_freq(ST_DVBCA_FREQ *pstFreq, unsigned char ucFreqSum)
{
	...
	{
		ret = app_popmsg(POP_DIALOG_X_START, POP_DIALOG_Y_START,"发现新节目,是否进行搜索?",POPMSG_TYPE_YESNO);
	}
	else if(0 == strcmp(osd_language,LANGUAGE_ENGLISH))
	{
		ret = app_popmsg(POP_DIALOG_X_START, POP_DIALOG_Y_START,"Find new program,whether search?",POPMSG_TYPE_YESNO);
	}
...
void app_search_startup_auto_scan(startup_search_para startuppara)
{
	...
	else
	{
		...
		GUI_SetProperty(NULL,"draw_now",NULL);
		{
			...
			searchFreList.app_qam_array[0]=qam;
			searchFreList.app_symb_array[0]=symbol_rate;
			searchFreList.app_fre_array[1] = app_flash_get_config_manual_search_freq();
			searchFreList.app_qam_array[1]=qam;
			searchFreList.app_symb_array[1]=symbol_rate;
			...
			switch(startuppara.nit_flag)
			{
				...
				//			GUI_SetInterface("flush", NULL);
					...
					{	
						if (0 == app_search_lock_tp(searchFreList.app_fre_array[1], searchFreList.app_symb_array[1],
									INVERSION_OFF, searchFreList.app_qam_array[1],2000))
...
void app_search_startup_auto_scan2(startup_search_para startuppara,uint16_t *fre,uint16_t size)
{
	uint32_t symbol_rate = 0;
	uint32_t qam = 0;
	uint32_t   panel_prog_cur=0;
...
void app_search_stop_ok_msg( search_ok_msg ok_msg)
{
	...
	else
	{
		...
		{
			if (LCN_STATE_ON == lcn_flag &&((ret == 2)||(ok_msg.app_search_ok_pomsg==NULL)))
			{
	...
	{
		if (TRUE == app_lcn_list_get_prog_pos(ok_msg.play_pos,&pos1))
		{
			pos = pos1;
		}
		else
...
void app_search_sat_tp_reply_msg(search_sat_tp_reply_msg  sat_tp_reply_msg)
{
	...
	sprintf((char*)buf,"%d MHz",params->frequency/1000);
...
#ifdef CUST_NEIMENG
if (strengthxml.panel_flag == 1)
	app_panel_show(PANEL_STRENGTH,&(value));								
#endif
