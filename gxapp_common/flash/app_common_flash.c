...
#define OEM_UPDATE_TYPE		("update_type")
...
* set update type 1 2
*/
uint32_t app_flash_set_oem_fe_update_type(char* Value)
{ 
	if (NULL == Value)
		return 0;
...
uint32_t app_flash_get_config_center_freq(void)
{
	...
	GxBus_ConfigGetInt(MAIN_FREQ, &config, DVB_CENTER_FRE);
	return config;
}
...
int32_t app_flash_get_config_center_freq_symbol_rate(void)
{
	...
	GxBus_ConfigGetInt(MAIN_FREQ_SYMRATE, &config,DVB_CENTER_SYMRATE);
...
int32_t app_flash_get_config_center_freq_qam(void)
{
	...
	GxBus_ConfigGetInt(MAIN_FREQ_QAM, &config,DVB_CENTER_QAM);
...
{
	int32_t config = 0;
	GxBus_ConfigGetInt(PVR_DURATION_KEY, &config,0);
	return config;
}
