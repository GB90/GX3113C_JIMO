...
void app_prog_init_playing(void)
{
	...
	gApp_record_prog.pos_in_group = 0xffffffff;
	memset(&gApp_old_prog,0,sizeof(play_prog));
	gApp_old_prog.pos_in_group = 0xffffffff;
}
...
void app_prog_record_playing_pos(uint32_t pos_in_group)
{
	...
	if (0xffffffff == gApp_record_prog.pos_in_group)
	{
		/*
		* record startup prog
		*/
	...
	gApp_record_prog.pos_in_group = pos_in_group;
...
#endif
	{
		app_prog_get_pos_in_group_by_id(pSysInfo.tv_prog_cur,&gApp_record_prog.pos_in_group);
	}
	...
	{
		app_prog_get_pos_in_group_by_id(pSysInfo.hd_prog_cur,&gApp_record_prog.pos_in_group);
	}
#endif
...
#endif
	{
		app_prog_get_pos_in_group_by_id(pSysInfo.radio_prog_cur,&gApp_record_prog.pos_in_group);
	}
...
void app_prog_get_pos_in_group_by_id(uint32_t prog_id,uint32_t *ppos)
{
	GxBusPmDataProg prog_data;
	uint32_t prog_num,i;
...
void app_prog_get_playing_pos_in_group(uint32_t *ppos)
{
	GxBusPmDataProg prog_data;
	GxBusPmViewInfo sysinfo;
	uint32_t prog_num,i;
...
//					app_book_delete_prog(prog.id);
...
#if (1 == DVB_HD_LIST)
			else if (GXBUS_PM_PROG_HD_SERVICE == sys.stream_type)
				{
					if (sys.hd_prog_cur != prog.id)
						{
							sys.hd_prog_cur = prog.id;	
							GxBus_PmViewInfoModify(&sys);
						}					
			...
			{
				if (sys.radio_prog_cur != prog.id)
					{
						sys.radio_prog_cur = prog.id;
						GxBus_PmViewInfoModify(&sys);
					}
			}
			app_prog_record_playing_pos(value);		
